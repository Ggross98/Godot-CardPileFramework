...
public partial class CardPileUI : Control
{
    [Signal]
    ...
    [Signal]
    public delegate void CardRemovedFromDropzoneEventHandler(CardDropzone dropzone, CardUI card);
    [Signal]
    public delegate void CardAddedToDropzoneEventHandler(CardDropzone dropzone, CardUI card);
    [Signal]
    ...
    [Signal]
    public delegate void CardRemovedFromGameEventHandler(CardUI card);
    ...
    private Array<CardUI> _drawPile = new Array<CardUI>();
    private Array<CardUI> _handPile = new Array<CardUI>();
    private Array<CardUI> _discardPile = new Array<CardUI>();
    ...
    public void SetCardPile(CardUI card, Piles pile)
    {
    ...
    public void SetCardDropzone(CardUI card, CardDropzone dropzone)
    {
        MaybeRemoveCardFromAnyPiles(card);
        MaybeRemoveCardFromAnyDropzones(card);
    ...
    private void RemoveCardFromGame(CardUI card)
    {
        MaybeRemoveCardFromAnyPiles(card);
        MaybeRemoveCardFromAnyDropzones(card);
    ...
    public Array<CardUI> GetCardsInPile(Piles pile)
    {
        if (pile == Piles.DiscardPile)
            return [.. _discardPile];
        ...
        return new Array<CardUI>();
    }

    public CardUI GetCardInPileAt(Piles pile, int index)
    {
    ...
    private void MaybeRemoveCardFromAnyPiles(CardUI card)
    {
    ...
    private void MaybeRemoveCardFromAnyDropzones(CardUI card)
    {
    ...
    public CardDropzone GetCardDropzone(CardUI card)
    {
        var allDropzones = new Array<CardDropzone>();
        GetDropzones(GetTree().Root, "CardDropzone", allDropzones);
        foreach (var dropzone in allDropzones)
    ...
    private void ResetCardCollection()
    {
        ...
        {
            MaybeRemoveCardFromAnyPiles(child as CardUI);
            MaybeRemoveCardFromAnyDropzones(child as CardUI);
            RemoveCardFromGame(child as CardUI);
        }
    ...
    public bool IsCardInHand(CardUI cardUi)
    {
    ...
    public new void Draw(int numCards = 1)
    {
        for (int i = 0; i < numCards; i++)
        {
            ...
            {
                var dupeDiscard = new Array<CardUI>(_discardPile);
    ...
    public void SortHand(Func<CardUI, CardUI> sortFunc)
    {
        _handPile.OrderBy(sortFunc);
        ResetTargetPositions();
    }

    private CardUI CreateCardUI(Dictionary jsonData)
    {
        var cardUi = ExtendedCardUI.Instantiate<CardUI>();
        
        // Card data initialilzation
        var script = (CSharpScript)ResourceLoader.Load(jsonData["resource_script_path"].As<string>());
        var cardData = script.New().As<CardUIData>();
    ...
    private void OnCardHovered(CardUI cardUi) => EmitSignal(nameof(CardHovered), cardUi);
    private void OnCardUnhovered(CardUI cardUi) => EmitSignal(nameof(CardUnhovered), cardUi);
    private void OnCardClicked(CardUI cardUi) => EmitSignal(nameof(CardClicked), cardUi);
    private void OnCardDropped(CardUI cardUi) => EmitSignal(nameof(CardDropped), cardUi);
}
